...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            this.form = FormXMLLoader.loadFromFile( this.getAssets().open( DATA_ASSET ) );
    ...
    private void onTalk()
    {
        if ( !this.ttEngine.isSpeaking() ) {
            ...
            final StringBuilder TEXT =
                    new StringBuilder( LBL_QUESTION.getText().length() );

            TEXT.append( LBL_QUESTION.getText() );

            if ( RADIO_GROUP.getVisibility() == View.VISIBLE ) {
                TEXT.append( '\n' );

                for(int i = 0; i < RADIO_GROUP.getChildCount(); ++i) {
                    final RadioButton BT =
                            (RadioButton) RADIO_GROUP.getChildAt( i );
    ...
    private void showFormEnd()
    {
        ...
        END_TEXT.append( this.player.getResultsAsText() );
    ...
    private void saveJSON()
    {
        ...
        try (FileOutputStream f = this.openFileOutput( "output.json", Context.MODE_PRIVATE ) ) {
            DataOutputStream STREAM = new DataOutputStream( f );
            final PrintWriter WR = new PrintWriter( STREAM );
            new FormJSONSaver( this.player ).saveToJSON( WR );
            WR.flush();
            WR.close();
            STREAM.close();
        } catch (IOException e) {
    ...
    private void buildNextButton()
    {
        ...
        BUTTON_NEXT.setOnClickListener( (v) -> {
            ...
            if ( SELF.fetchAnswerFor( this.getCurrentQuestion() ) ) {
                ...
                if ( this.player.gotoNextQuestion() ) {
                    SELF.showCurrentQuestion();
                } else {
                    SELF.showFormEnd();
                }
    ...
    private boolean fetchAnswerFor(final Question Q)
    {
        ...
        if ( toret ) {
            if ( !this.player.getCurrentBranch().getId().equals( "data" ) )
            {
                this.player.setEnteredVal( value );
            }

            // Store the value
            this.player.getRepo().setValue( ID, value );
        }
    ...
    {
        final Repo REPO = this.player.getRepo();
        final Branch BR = this.player.getCurrentBranch();
        final Question Q = this.player.getCurrentQuestion();

        if ( Q.isEnd() ) {
            if ( BR.getId().equals( "data" ) ) {
                // PCD-JMP_SCREEN-1316
                this.player.changeToBranch( "screen" );
            }
            else
            if ( BR.getId().equals( "screen" ) ) {
                int totalScreen = REPO.calcTotalScreen();

                if ( totalScreen >= 2 ) {
                    // PCD-JGTE_2_migraine-1559
                    this.player.changeToBranch( "migraine" );
                }
            }
        }
    }
    ...
    private Question getCurrentQuestion()
    {
        ...
        if ( this.player == null ) {
            ...
            toret = this.player.getCurrentQuestion();
